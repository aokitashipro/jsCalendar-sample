...
function add_calendar(wrapper, year, month) {
	...
	// カレンダー追加後にクリックイベントを追加
	add_click_event_to_calendar(wrapper);
}

/**
 * 指定した年月のカレンダーのヘッダー要素を生成して返す
 * @param {object} wrapper - カレンダー追加親要素
 * @param {number} year    - 年の指定
 * @param {number} month   - 月の指定
 */
...
function generate_month_calendar(year, month) {
	...
	var i = calendarData[calendarData.length - 1]['weekday']; // 末日曜日取得
	// カレンダー上の末日より後を埋める
	while(i < 6) {
		i++;
		calendarData.push({
	...
	for (var i = 0; i < calendarData.length; i++) {
		...
		if(calendarData[i]['day'] !== '') {
			insertData += '<td data-day="' + year + '-' + month + '-' + calendarData[i]['day'] + '">';
		} else {
			insertData += '<td>';
		}
...
function get_month_calendar(year, month) {
	var firstDate = new Date(year, (month - 1), 1); // 指定年月初日の情報
	var lastDay = new Date(year, (firstDate.getMonth() + 1), 0).getDate(); // 
	var weekday = firstDate.getDay(); // 指定年月初日の曜日

	var calendarData = []; // カレンダー情報格納
 ...
 */
function add_click_event_to_calendar(calendar) {
	var input = document.getElementById('calendar-date'); // IDで指定
	var targets = calendar.getElementsByTagName('td'); // タグ要素
	// tdの数の分だけforを回す
	for (var i = 0; i < targets.length; i++) {
		targets[i].addEventListener('click', function(e) {
			...
			if(e.target.dataset.day !== undefined) {
				...
				input.value = e.target.dataset.day;
			}
		}, false);
	}
}
